...
// after it has been clicked heart button is going to check the condition in getLi if condition is false, it means it hasn't found the li with that counter number 
// if it is false it is going to create new li for the respective counter number 
// and appends it to Ul
// else it will just keep incrementing the li's child which is span 
heart.addEventListener("click",(e)=>{
    ...
    if(!getLi){  
        ...
        newli.dataset.num = counter.innerText; // assigns the counter innerText for newLi and creates another data-num 
        newli.innerHTML = `${counter.innerText} has been liked <span>1</span> times`; //will add to innerHTML
        like.append(newli);
    } else {
